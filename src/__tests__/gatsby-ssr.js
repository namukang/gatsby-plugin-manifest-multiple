...
describe(`gatsby-plugin-manifest`, () => {
  ...
  describe(`Manifest Link Generation`, () => {
    ...
    it.skip(`Adds "shortcut icon" and "manifest" links and "theme_color" meta tag to head`, () => {
      onRenderBody(ssrArgs, {
        icon: true,
        theme_color: `#000000`,
      });
    ...
    const appArgs = [
      ...
      },
      {
        ...ssrArgs,
        pathname: `/app/`,
        testName: `Adds correct (custom app) "manifest" link to head`,
      },
    ];

    appArgs.forEach(({ testName, ...args }) =>
      it.skip(testName, () => {
        onRenderBody(args, {
          name: `My Website`,
          start_url: `/`,
