...
describe(`Test plugin manifest options`, () => {
  ...
  it.skip(`does file name based cache busting`, async () => {
    fs.existsSync.mockReturnValueOnce(true);
  ...
  it(`generates all app versions`, async () => {
    const pluginSpecificOptions = {
      childApps: [
        {
          start_url: `/de/`,
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      path.join(`public`, `manifest_de.webmanifest`),
      expect.anything()
    );
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      path.join(`public`, `manifest_es.webmanifest`),
      expect.anything()
    );
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      path.join(`public`, `manifest_custom-app.webmanifest`),
      expect.anything()
    );
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      path.join(`public`, `manifest.webmanifest`),
  ...
  it(`merges default and language options`, async () => {
    ...
    const expectedResults = childApps.map(appManifest => {
      return { ...manifestOptions, ...appManifest };
    });
